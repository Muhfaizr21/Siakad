    ...
    >
      ...
      <div className="p-4 flex-1 flex flex-col text-sm text-slate-500">
        <div className="flex items-start gap-2">
          <MapPin className="size-4 text-[var(--theme-secondary)] shrink-0 mt-0.5" />
          {gmaps_url ? (
            <a 
              href={gmaps_url}
              target="_blank"
              rel="noopener noreferrer"
              className="line-clamp-2 hover:text-[var(--theme-primary)] hover:underline transition-colors"
            >
...
export default function LocationsSection({ settings, hideLink = false }) {
  ...
  if (settings?.locations_items) {
    try {
      locationsList = JSON.parse(settings.locations_items)
    } catch (e) {
