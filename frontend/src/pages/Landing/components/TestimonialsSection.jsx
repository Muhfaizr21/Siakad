...
function TestimonialCard({ name, role, content, index }) {
  ...
  return (
    ...
    >
      <Quote className="size-8 text-[var(--theme-secondary)]/20 absolute top-6 right-6" />
      <div className="flex gap-1 mb-4">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="size-4 fill-[var(--theme-secondary)] text-[var(--theme-secondary)]" />
        ))}
      </div>
      <p className="text-slate-600 leading-relaxed mb-6">&ldquo;{content}&rdquo;</p>
      <div className="flex items-center gap-3">
        <div className="size-10 rounded-full bg-[var(--theme-primary)] flex items-center justify-center text-white font-bold text-sm">
...
export default function TestimonialsSection({ settings }) {
  ...
  if (settings?.testimonials_items) {
    try {
      testimonialsList = JSON.parse(settings.testimonials_items)
    } catch (e) {
