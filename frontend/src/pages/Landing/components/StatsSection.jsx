...
function StatCard({ iconName, value, label, index }) {
  ...
  return (
    ...
    >
      <div className="size-12 rounded-xl bg-[var(--theme-primary)]/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
        <Icon className="size-6 text-[var(--theme-primary)]" />
      </div>
      <p className="text-2xl font-extrabold font-headline text-[var(--theme-primary)] mb-1">
        {value}
      </p>
...
export default function StatsSection({ settings }) {
  ...
  if (settings?.stats_section_items) {
    try {
      statsList = JSON.parse(settings.stats_section_items)
    } catch (e) {
