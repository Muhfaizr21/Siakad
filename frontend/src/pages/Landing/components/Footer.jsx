...
export default function Footer({ settings }) {
  ...
  const socialLinks = (() => {
    try {
      return JSON.parse(settings?.footer_socials || '[]');
    } catch {
      return [];
    }
  })();
  ...
  const campusList = (() => {
    try {
      return JSON.parse(settings?.locations_items || '[]');
    } catch {
      return campuses;
    }
  })();
    ...
    <footer className="bg-[var(--theme-primary)] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          {/* Campuses */}
          <div>
            <h4 className="text-sm font-bold uppercase tracking-widest mb-5 text-[var(--theme-secondary)]">
              Kampus Kami
            </h4>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {programs.map((prog) => (
                <li key={prog.name}>
                  ...
                  >
                    <ChevronRight className="size-3 text-[var(--theme-secondary)] group-hover:translate-x-0.5 transition-transform" />
                    {prog.name}
                  </Link>
                </li>
              ))}
            ...
            <h4 className="text-sm font-bold uppercase tracking-widest mb-5 mt-8 text-[var(--theme-secondary)]">
              Lainnya
            </h4>
            <ul className="space-y-3">
              {[
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <Phone className="size-5 text-[var(--theme-secondary)] shrink-0 mt-0.5" />
                <div>
              ...
              <li className="flex items-start gap-3">
                <Mail className="size-5 text-[var(--theme-secondary)] shrink-0 mt-0.5" />
                <div>
            ...
            <div className="mt-8">
              <h5 className="text-sm font-bold mb-3 text-white/80">
                Ikuti Info Terbaru
              </h5>
              <form
              ...
              >
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-white/10 border border-white/10 text-sm text-white placeholder-white/40 focus:outline-none focus:border-[var(--theme-secondary)] transition-all"
                />
                <button
                  type="submit"
                  className="px-4 py-2.5 rounded-xl text-sm font-bold transition-all"
                  style={{ background: 'var(--theme-secondary)', color: 'var(--theme-primary)' }}
                >
                  Kirim
                </button>
              </form>
