...
export default function Tentang() {
  ...
  let leaders = [];
  try {
    leaders = JSON.parse(settings?.tentang_leaders || '[]');
  } catch (e) {}
  if (!leaders.length) {
    leaders = [
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <AnimatedSection className="p-8 rounded-2xl bg-white border border-slate-100 shadow-sm">
              <div className="size-14 rounded-xl bg-[var(--theme-primary)]/5 flex items-center justify-center mb-5">
                <Eye className="size-7 text-[var(--theme-primary)]" />
              </div>
              <h2 className="text-2xl font-extrabold font-headline text-[var(--theme-primary)] mb-4">Visi</h2>
            ...
            <AnimatedSection className="p-8 rounded-2xl bg-white border border-slate-100 shadow-sm">
              <div className="size-14 rounded-xl bg-[var(--theme-primary)]/5 flex items-center justify-center mb-5">
                <Target className="size-7 text-[var(--theme-primary)]" />
              </div>
              <h2 className="text-2xl font-extrabold font-headline text-[var(--theme-primary)] mb-4">Misi</h2>
              <ul className="space-y-3 text-slate-600">
                {misi.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className="size-1.5 rounded-full bg-[var(--theme-secondary)] mt-2.5 shrink-0" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      <section className="py-16 lg:py-24 bg-slate-50/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection className="max-w-3xl mx-auto text-center">
            <p className="text-sm font-bold uppercase tracking-widest text-[var(--theme-secondary)] mb-3">Sejarah</p>
            <h2 className="text-2xl sm:text-3xl font-extrabold font-headline text-[var(--theme-primary)] mb-6">
              Perjalanan Kami
            </h2>
            ...
            {leaders.map((leader, i) => (
              <AnimatedSection key={leader.name}>
                <div className="p-6 rounded-2xl bg-white border border-slate-100 shadow-sm text-center group hover:shadow-lg hover:-translate-y-1 transition-all">
                  <div className="size-20 rounded-full bg-[var(--theme-primary)] flex items-center justify-center mx-auto mb-4">
                    <Award className="size-8 text-white" />
                  </div>
                  <h3 className="font-bold font-headline text-[var(--theme-primary)] text-sm mb-1">
                    {leader.name}
                  </h3>
                  <p className="text-xs text-[var(--theme-secondary)] font-semibold">{leader.role}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
