...
function FacultyCard({ faculty, index }) {
  ...
  const color = faculty.color || 'bg-[var(--theme-primary)]'
    ...
    >
      ...
      <div className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-bold font-headline text-[var(--theme-primary)] text-base">
            {faculty.name}
          </h3>
        </div>
          ...
          {faculty.programs?.map((prog, idx) => {
            ...
            return (
              <li key={idx} className="flex items-center gap-2 text-sm text-slate-600">
                <div className="size-1.5 rounded-full bg-[var(--theme-secondary)]" />
                {progName}
              </li>
            );
          })}
...
export default function ProgramStudi() {
  ...
  try {
    ...
    faculties = parsed.map(f => ({
      ...
      color: f.color || 'bg-[var(--theme-primary)]'
    }));
  } catch(e) {}
  if (!faculties.length) {
    faculties = [
      ...
      <section className="py-16 lg:py-24 bg-slate-50/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <Icons.GraduationCap className="size-12 text-[var(--theme-secondary)] mx-auto mb-4" />
            <h2 className="text-2xl font-extrabold font-headline text-[var(--theme-primary)] mb-3">
              Tertarik Bergabung?
            </h2>
            <p className="text-slate-500 max-w-lg mx-auto mb-8">
              ...
              <Link
                to="/register"
                className="inline-flex items-center gap-2 px-8 py-4 rounded-xl font-bold text-white bg-[var(--theme-primary)] hover:bg-[#152F58] transition-all hover:scale-105 active:scale-95"
            >
