  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (res?.data?.syarat_ketentuan) {
          setContent(res.data.syarat_ketentuan);
        }
      } catch (error) {
        console.error('Failed to load syarat & ketentuan:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="bg-slate-50 min-h-screen pt-24 pb-16">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className="mb-8"
        >
          <Link to="/" className="inline-flex items-center text-sm font-medium text-[var(--theme-primary)] hover:text-blue-800 transition-colors mb-4">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Kembali ke Beranda
          </Link>
          <h1 className="text-4xl font-headline font-bold text-gray-900 mb-4">Syarat & Ketentuan</h1>
        </motion.div>
