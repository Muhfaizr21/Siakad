    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-3xl border border-[#e5e5e5] p-6 md:p-8 shadow-sm">
         <div className="flex items-center gap-3 mb-8">
            <div className="bg-[#fef2f2] text-[#ef4444] p-3 rounded-2xl">
               <KeyRound size={24} />
            </div>
            <div>
         ...
         <form onSubmit={handleSubmit((data) => passwordMutation.mutate(data))} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2 relative">
               ...
               {errors.old_password && <p className="text-xs font-bold text-red-500">{errors.old_password.message}</p>}
            </div>
               ...
               <div className="h-1.5 w-full bg-[#f5f5f5] rounded-full overflow-hidden mt-2">
                  <div 
                    className={`h-full transition-all duration-500 ${
                        ...
                        strength < 100 ? 'bg-yellow-500' : 'bg-green-500'
                    }`} 
            ...
            <div className="space-y-2">
               ...
               {errors.confirm_password && <p className="text-xs font-bold text-red-500">{errors.confirm_password.message}</p>}
            </div>
            ...
            <div className="md:col-span-3 pt-4 border-t border-[#f5f5f5] flex justify-end">
               <button 
                ...
                className="bg-[#171717] text-white py-3 px-8 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-black transition-all disabled:opacity-50"
               >
      ...
      <div className="bg-white rounded-3xl border border-[#e5e5e5] p-6 md:p-8 shadow-sm">
         <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-3">
               <div className="bg-[#eff6ff] text-[#3b82f6] p-3 rounded-2xl">
                  <Monitor size={24} />
               </div>
               <div>
            ...
            <button 
                className="text-xs font-black text-red-500 uppercase tracking-widest px-4 py-2 bg-red-50 rounded-xl hover:bg-red-100 transition-colors"
                  ...
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <div className="flex items-center gap-2">
                           ...
                           {s.is_current && <span className="bg-[#f0fdf4] text-[#16a34a] text-[10px] font-black uppercase px-2 py-0.5 rounded-full">Perangkat Ini</span>}
                        </div>
                        <p className="text-xs font-medium text-[#a3a3a3]">{s.location} <span className="mx-1">•</span> {s.last_active}</p>
                     </div>
                  </div>
                  {!s.is_current && (
                    <button className="p-2 text-[#a3a3a3] hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                       <LogOut size={18} />
                    </button>
                  )}
               </div>
      ...
      <div className="bg-white rounded-3xl border border-[#e5e5e5] shadow-sm overflow-hidden">
         <div className="p-6 md:p-8 flex items-center justify-between border-b border-[#f5f5f5]">
            <div className="flex items-center gap-3">
               <div className="bg-[#f5f3ff] text-[#8b5cf6] p-3 rounded-2xl">
                  <History size={24} />
               </div>
               <h3 className="text-xl font-extrabold font-headline">Riwayat Login</h3>
            </div>
            <button className="text-sm font-bold text-[#f97316] hover:underline">Lihat Semua</button>
         </div>
                    ...
                    <tr key={h.id} className="hover:bg-[#fafafa] transition-colors">
                       ...
                       <td className="px-8 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                            h.status === 'Berhasil' ? 'bg-[#f0fdf4] text-[#16a34a] border-[#dcfce7]' : 'bg-[#fef2f2] text-[#ef4444] border-[#fee2e2]'
                          }`}>
                             {h.status}
                          </span>
                       </td>
