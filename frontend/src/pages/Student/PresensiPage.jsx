        ...
        <CardContent className="p-8 flex flex-col items-center text-center gap-6 relative">
          ...
          {status === 'loading' && (
            <div className="flex flex-col items-center gap-6 my-6">
              ...
              <div className="space-y-1.5">
                <p className="font-black text-slate-800 text-sm uppercase tracking-wider">Memproses Presensi...</p>
                <p className="text-xs text-slate-400 font-medium">Sistem sedang memverifikasi identitas dan koordinasi kegiatan Anda.</p>
              </div>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="flex flex-col items-center gap-6 my-4 w-full">
              <div className="w-20 h-20 rounded-full bg-emerald-500 flex items-center justify-center text-white shadow-xl shadow-emerald-500/20 animate-bounce">
                <span className="material-symbols-outlined text-4xl font-bold">check</span>
              </div>
              
              <div className="space-y-2 w-full">
                <h3 className="font-black text-emerald-600 text-lg uppercase tracking-wider font-headline">Presensi Berhasil!</h3>
                <div className="p-4 bg-emerald-50/30 border border-emerald-100 rounded-2xl text-left space-y-1.5">
                  <p className="text-[10px] font-black text-emerald-700 tracking-wider uppercase leading-none">Data Terdaftar</p>
                  <p className="text-sm font-black text-slate-900 font-headline truncate leading-tight">{student?.Nama || user?.Nama || 'Mahasiswa BKU'}</p>
                  <p className="text-xs font-bold text-slate-500 leading-none">NIM: {student?.NIM || '-'}</p>
                  <p className="text-xs font-bold text-slate-400 leading-none mt-1">Presensi direkam pada: {new Date().toLocaleTimeString('id-ID')}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {status === 'error' && (
            <div className="flex flex-col items-center gap-6 my-4 w-full">
              ...
              <div className="space-y-2 w-full">
                <h3 className="font-black text-rose-600 text-lg uppercase tracking-wider font-headline">Presensi Gagal</h3>
          ...
          <div className="w-full pt-4 border-t border-slate-100 flex flex-col gap-3">
            <NavLink to="/student/dashboard" className="w-full">
              <Button className="w-full h-12 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-black text-xs tracking-wider uppercase shadow-lg">
