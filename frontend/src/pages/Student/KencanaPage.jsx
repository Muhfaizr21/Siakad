...
export default function KencanaPage() {
  ...
  const getStatusColor = (status) => {
    ...
    if (['passed', 'approved', 'completed', 'active'].includes(s)) return 'text-[var(--theme-success)]';
    if (['remedial', 'not_eligible', 'rejected'].includes(s)) return 'text-[var(--theme-error)]';
    if (['conditional_pass', 'submitted', 'pending', 'in_progress'].includes(s)) return 'text-[var(--theme-warning)]';
  ...
  return (
    ...
    >
      {/* Overview Section */}
      <section className="overflow-hidden rounded-2xl border border-border bg-[var(--theme-primary)] text-[var(--theme-text-on-primary)] shadow-md mb-6">
        <div className="grid gap-6 p-6 md:grid-cols-[1.4fr_0.8fr] md:p-8">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold md:text-3xl font-headline text-inherit">Status: {dashboardData?.graduation_status ? <span className={getStatusColor(dashboardData.graduation_status)}>{dashboardData.graduation_status.replaceAll('_', ' ')}</span> : 'Belum Mulai'}</h2>
          ...
          <div className="rounded-2xl bg-white/10 p-5 backdrop-blur flex flex-col justify-center">
            ...
            <p className="mt-2 text-4xl font-bold font-headline">{Number(dashboardData?.temporary_final_score || 0).toFixed(1)}</p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {/* WHAT → Komponen Nilai */}
        <div className="rounded-2xl border border-border bg-[var(--theme-surface)] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl bg-[var(--theme-warning)]/10 flex items-center justify-center text-[var(--theme-warning)]">
              <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>grade</span>
            </div>
            <div>
              <h3 className="text-xs font-bold text-[var(--theme-text-muted)] uppercase tracking-widest">Komponen Nilai</h3>
              <p className="text-[10px] text-[var(--theme-text-muted)]">WHAT: Bobot penilaian</p>
            </div>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-[var(--theme-text-muted)] w-20">{item.name}</span>
                <div className="flex-1 h-5 bg-[var(--theme-border-muted)] rounded-full overflow-hidden">
                  <div className={cn('h-full rounded-full transition-all', item.color)} style={{width:`${item.bobot}%`}}/>
                </div>
                <span className="text-xs font-bold text-[var(--theme-text)] w-8 text-right">{item.bobot}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-[var(--theme-surface)] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-xs font-bold text-[var(--theme-text-muted)] uppercase tracking-widest">Tahap Aktif</h3>
              <p className="text-[10px] text-[var(--theme-text-muted)]">WHEN: Tahapan saat ini</p>
            </div>
          </div>
          <div className="text-center py-4">
            <span className="text-2xl font-bold text-[var(--theme-primary)]">{dashboardData?.active_stage?.name || '-'}</span>
        ...
        <div className="rounded-2xl border border-border bg-[var(--theme-surface)] p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl bg-[var(--theme-success)]/10 flex items-center justify-center text-[var(--theme-success)]">
              <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>verified</span>
            </div>
            <div>
              <h3 className="text-xs font-bold text-[var(--theme-text-muted)] uppercase tracking-widest">Criteria</h3>
              <p className="text-[10px] text-[var(--theme-text-muted)]">HOW: Syarat kelulusan</p>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2 bg-[var(--theme-success)]/10 rounded-xl">
              <span className="text-xs font-medium text-[var(--theme-success)]">Nilai Minimum</span>
              <span className="text-sm font-bold text-[var(--theme-success)]">70.0</span>
            </div>
            <div className="flex items-center justify-between p-2 bg-[var(--theme-info)]/10 rounded-xl">
              <span className="text-xs font-medium text-[var(--theme-info)]">Remedial</span>
              <span className="text-sm font-bold text-[var(--theme-info)]">{dashboardData?.needs_remedial ? 'Perlu' : 'Tidak'}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.6fr_1fr]">
        <div className="space-y-6">
          {/* Timeline Cards */}
          <div className="rounded-2xl border border-border bg-[var(--theme-surface)]/85 p-6 shadow-sm">
              ...
              {sortedStages.map((stage, index) => {
                ...
                return (
                  <CardComponent 
                    key={stage.id} 
                    {...linkProps} 
                    className={`group relative grid gap-4 rounded-2xl border ${stage.status === 'active' ? 'border-[var(--theme-primary)] bg-[var(--theme-primary)]/10 shadow-md' : 'border-border bg-[var(--theme-surface)]'} p-5 transition-all ${user?.role === 'super_admin' ? '' : 'hover:-translate-y-1 hover:shadow-lg'} md:grid-cols-[auto_1fr_auto]`}
                  >
                    {stage.status === 'active' && (
          ...
          {user?.role !== 'super_admin' && (
            <div className="grid gap-4 md:grid-cols-3">
              <Link to="/student/kencana/score" className="group rounded-2xl border border-border bg-[var(--theme-surface)] p-5 transition hover:bg-[var(--theme-border-muted)] hover:-translate-y-1 hover:shadow-sm">
              ...
              <Link to="/student/kencana/invitations" className="group rounded-2xl border border-border bg-[var(--theme-surface)] p-5 transition hover:bg-[var(--theme-border-muted)] hover:-translate-y-1 hover:shadow-sm">
              ...
              <Link to="/student/kencana/attendance" className="group rounded-2xl border border-border bg-[var(--theme-surface)] p-5 transition hover:bg-[var(--theme-border-muted)] hover:-translate-y-1 hover:shadow-sm">
        ...
        <div className="space-y-6">
          ...
          {user?.role !== 'super_admin' && dashboardData?.last_activity?.id && (
            <div className="rounded-2xl border border-border bg-[var(--theme-surface)] p-6 shadow-sm relative overflow-hidden">
          ...
          {/* Transparansi Status */}
          <div className="rounded-2xl border border-border bg-[var(--theme-surface)] p-6 shadow-sm">
