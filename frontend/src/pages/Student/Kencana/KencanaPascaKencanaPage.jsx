...
export default function KencanaScorePage() {
  ...
  if (isLoading) return <KencanaShell title="Nilai Kencana" breadcrumbs={[{ label: 'Nilai & Kelulusan' }]}><LoadingPanel /></KencanaShell>;
  if (isError) return <KencanaShell title="Nilai Kencana" breadcrumbs={[{ label: 'Nilai & Kelulusan' }]}><ErrorPanel message="Gagal memuat nilai." /></KencanaShell>;
  const score = data?.score || {};
  return (
    <KencanaShell title="Nilai dan Kelulusan" subtitle="Nilai akhir = Kognitif 25% + Psikomotor 35% + Afektif 40%. Kehadiran tetap menjadi syarat wajib." breadcrumbs={[{ label: 'Nilai & Kelulusan' }]}>
