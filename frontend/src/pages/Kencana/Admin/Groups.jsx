  ...
  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 min-h-screen bg-transparent font-body max-w-7xl mx-auto space-y-6">
      ...
      <DialogModal
        ...
        description="Silakan tentukan nomor, nama, kuota, dan mentor pembimbing untuk kelompok."
        maxWidth="max-w-2xl"
      >
        <form onSubmit={saveGroup} className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Nomor Kelompok</span>
              <input required type="number" min="1" value={form.group_number} onChange={e => setForm({ ...form, group_number: e.target.value })} placeholder="No. Kel." className="w-full h-10 px-4 rounded-xl bg-[var(--theme-bg)] border border-[var(--theme-border)] text-sm font-semibold focus:outline-none focus:border-[var(--theme-primary)]" />
            </div>
            <div className="md:col-span-2 space-y-1">
              <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Nama Kelompok</span>
              ...
              }} placeholder="Contoh: Praja" className="w-full h-10 px-4 rounded-xl bg-[var(--theme-bg)] border border-[var(--theme-border)] text-sm font-semibold focus:outline-none focus:border-[var(--theme-primary)]" />
            </div>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Kode Kelompok</span>
            <input value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} placeholder="Contoh: PRAJA-01" className="w-full h-10 px-4 rounded-xl bg-[var(--theme-bg)] border border-[var(--theme-border)] text-sm font-semibold focus:outline-none focus:border-[var(--theme-primary)]" />
          </div>
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Deskripsi</span>
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Deskripsi kelompok..." rows="3" className="w-full p-4 rounded-xl bg-[var(--theme-bg)] border border-[var(--theme-border)] text-sm font-medium focus:outline-none focus:border-[var(--theme-primary)]" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {!isFacultyScoped && (
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Scope</span>
                <SelectField
            ...
            {((!isFacultyScoped && form.scope_type === 'faculty') || isFacultyScoped) && (
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Fakultas</span>
                <SelectField
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2 space-y-1">
              <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Mentor/DP</span>
              <SelectField
            ...
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Kuota</span>
              <input type="number" min="1" value={form.capacity} onChange={e => setForm({ ...form, capacity: e.target.value })} className="w-full h-10 px-4 rounded-xl bg-[var(--theme-bg)] border border-[var(--theme-border)] text-sm font-semibold focus:outline-none focus:border-[var(--theme-primary)]" />
            </div>
          </div>
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-[var(--theme-text-muted)] uppercase tracking-wider pl-1">Status Keaktifan</span>
            <SelectField
              value={form.status}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-[var(--theme-border-muted)]">
            <button type="button" onClick={() => setShowForm(false)} className="px-5 py-2 rounded-xl font-bold text-[var(--theme-text-muted)] hover:bg-[var(--theme-bg)] transition-colors">Batal</button>
            <button type="submit" className="px-6 py-2 h-10 bg-[var(--theme-primary)] hover:bg-[var(--theme-primary-hover)] text-white font-bold rounded-xl transition-colors">Simpan</button>
          </div>
        </form>
      </DialogModal>
    </div>
