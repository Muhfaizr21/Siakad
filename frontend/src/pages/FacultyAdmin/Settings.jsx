...
export default function Settings() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    setSubmitting(true)
    try {
      const { data } = await api.put('/faculty/profile', { 
        email: profile.email,
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      const { data } = await api.post('/faculty/profile/upload-avatar', formData, {
  ...
  const handleAvatarDelete = async () => {
    try {
      setUploadingAvatar(true)
      const { data } = await api.delete('/faculty/profile/avatar')
      if (data.success) {
