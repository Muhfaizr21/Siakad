...
const studentColumns = [
  ...
  { label: 'Program Studi', key: 'Mahasiswa.ProgramStudi.Nama', sortable: true, render: (val) => <span className="text-sm text-[var(--theme-text-subtle)] font-medium">{val || '—'}</span> },
  { label: 'Kehadiran', key: 'attendanceRate', sortable: true, render: (val) => <span className="font-black text-sm text-[var(--theme-text)] tabular-nums">{val || 0}%</span> },
  { label: 'Nilai', key: 'Nilai', sortable: true, render: (val) => <span className="font-black text-sm text-[var(--theme-primary)] tabular-nums">{val || 0}</span> },
  { label: 'Status', key: 'StatusKelulusan', sortable: true, render: (val) => {
      const st = getLulus(val);
      return (
        <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold border uppercase tracking-wider whitespace-nowrap', st.cls)}>
          <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', st.dot)} />{val || 'Proses'}
        </span>
      )
    }
  }
      ...
      <div>
        {activeTab === 'prodi' ? (
          ...
          <DataTable
            ...
            actions={(row) => (
              <button onClick={() => setSelected(row)} className="p-1.5 text-[var(--theme-text-muted)] hover:text-[var(--theme-primary)] hover:bg-[var(--theme-primary-light)] rounded-lg transition-colors">
