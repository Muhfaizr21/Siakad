...
export default function EditProdiPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch faculties
        const resF = await fetch('http://localhost:8000/api/faculty/faculties')
        const jsonF = await resF.json()
        if (jsonF.status === 'success') {
          setFaculties(jsonF.data)
        }

        // Fetch major detail
        const resM = await fetch(`http://localhost:8000/api/faculty/majors/${id}`)
        const jsonM = await resM.json()
        if (jsonM.status === 'success') {
          setFormData({
            name: jsonM.data.name,
            code: jsonM.data.code || "",
            facultyId: jsonM.data.facultyId.toString(),
            degreeLevel: jsonM.data.degreeLevel || "S1",
            akreditasi: jsonM.data.akreditasi || "B",
            kapasitas: jsonM.data.kapasitas || 0
          })
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/faculty/majors/${id}`, {
        method: 'PUT',
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-on-surface-variant">Perbarui informasi program studi</p>
              </div>
            </div>
