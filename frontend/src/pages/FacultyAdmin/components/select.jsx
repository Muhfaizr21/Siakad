...
const SelectItem = React.forwardRef((props, ref) => {
  ...
  return (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-default select-none items-center justify-between rounded-lg py-2.5 px-3 text-sm font-bold outline-none focus:bg-primary/5 focus:text-primary data-[disabled]:pointer-events-none data-[disabled]:opacity-50 transition-colors",
        className
      )}
      {...rest}
    >
      ...
      <SelectPrimitive.ItemIndicator>
        <Check className="h-3.5 w-3.5 ml-2 shrink-0 opacity-70" />
      </SelectPrimitive.ItemIndicator>
    </SelectPrimitive.Item>
  );
});
