        ...
        return (
          <div className="flex items-center gap-4 py-2 group/avatar">
            <Avatar className="h-11 w-11 rounded-xl border-2 border-white shadow-md transition-all group-hover/avatar:scale-110 overflow-hidden">
              {row.Foto || row.Pengguna?.Foto || row.foto || row.pengguna?.foto ? (
                <AvatarImage src={row.Foto || row.Pengguna?.Foto || row.foto || row.pengguna?.foto} className="object-cover size-full" />
              ) : (
        ...
        <DialogContent className="max-w-md p-0 overflow-hidden border-none shadow-2xl rounded-3xl bg-white animate-in zoom-in-95 duration-300">
          ...
          <div className="p-10 space-y-8">
             ...
             <footer className="flex gap-4 pt-2">
                ...
                <Button onClick={handleUpdateRole} disabled={isSubmitting} className="flex-[2] h-12 rounded-xl bg-neutral-900 text-white font-bold text-[10px] uppercase tracking-widest shadow-xl shadow-neutral-900/10 active:scale-95 transition-all border-none flex items-center justify-center gap-2">
                  {isSubmitting ? <span className="material-symbols-outlined animate-spin" style={{ fontSize: '14px' }} >sync</span> : <span className="material-symbols-outlined" style={{ fontSize: '14px' }} Check >security</span>} 
                  Commit Authority
                </Button>
             </footer>
          </div>
