...
const LecturerDirectory = () => {
    const lecturers = [
        { id: "100203001", nidn: "0402038801", name: "Dr. Eng. Alistair Vance", fac: "Fakultas Teknik", major: "Informatika", status: "Active", role: "Head of Dept" },
        { id: "100203002", nidn: "0412059002", name: "Prof. Dr. Siti Aminah", fac: "Fakultas Ekonomi", major: "Akuntansi", status: "Active", role: "Lecturer" },
        { id: "100203003", nidn: "0422018503", name: "Budi Santoso, M.H.", fac: "Fakultas Hukum", major: "Ilmu Hukum", status: "On Leave", role: "Assoc. Professor" },
    ];
          ...
          <main className="pl-80 flex flex-col min-h-screen w-full">
            ...
            <div className="p-8 space-y-8">
              <header className="flex justify-between items-end">
                <div>
                  <h1 className="text-3xl font-extrabold text-primary tracking-tight font-headline uppercase italic">Master Lecturer Database</h1>
                  <p className="text-secondary mt-1 font-medium italic">Unified institutional records for all faculty members and research personnel.</p>
                </div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-outline-variant/30 flex shadow-lg hover:border-primary/50 transition-all">
                  <div className="relative flex-1">
                      ...
                      <input className="w-full bg-white pl-20 pr-10 py-8 rounded-[2rem] text-xl font-bold text-primary placeholder:text-secondary/30 outline-none" placeholder="Search by NIDN, Name, or Faculty Homebase..." />
                  </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-surface-container-low/30 text-[10px] font-black uppercase tracking-[0.2em] text-secondary/70 italic">
                      ...
                      <th className="px-10 py-6">Homebase Unit</th>
                  ...
                  <tbody className="divide-y divide-outline-variant/10 font-body select-text">
                    {lecturers.map((lecturer, idx) => (
                      <tr key={idx} className="hover:bg-primary/[0.01] transition-all group">
                        <td className="px-10 py-6">
                            <div className="flex items-center gap-5">
                                ...
                                <div className="flex flex-col">
                                    <span className="font-extrabold text-primary group-hover:text-blue-700 transition-colors tracking-tight uppercase">{lecturer.name}</span>
                                    <span className="text-[10px] text-secondary/60 font-black tracking-widest uppercase italic">{lecturer.fac}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td className="px-10 py-6 text-sm text-secondary font-bold font-headline">{lecturer.major}</td>
                        <td className="px-10 py-6">
                             <span className="text-xs font-bold text-primary uppercase tracking-widest">{lecturer.role}</span>
                        </td>
                        <td className="px-10 py-6">
                            <div className="flex items-center gap-2.5">
                                <div className={`w-2 h-2 rounded-full ${lecturer.status === 'Active' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-amber-500'}`}></div>
                                <span className={`text-[10px] font-black uppercase tracking-widest ${lecturer.status === 'Active' ? 'text-emerald-700' : 'text-amber-600'}`}>{lecturer.status}</span>
                            </div>
                        </td>
