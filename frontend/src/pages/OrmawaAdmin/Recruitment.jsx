  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 py-8 md:px-8 xl:px-12 space-y-8 font-body">
      ...
      <div className="flex border-b border-slate-200 overflow-x-auto no-scrollbar gap-6">
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-1.5 ${
            activeTab === 'pengaturan' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-1.5 ${
            activeTab === 'form-builder' ? 'border-violet-600 text-violet-600' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-1.5 ${
            activeTab === 'pending' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-bold border-b-2 transition-all whitespace-nowrap flex items-center gap-1.5 ${
            activeTab === 'riwayat' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'
          }`}
        >
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'pending' && (
          <Card className="border border-slate-200 shadow-sm overflow-hidden bg-white rounded-3xl">
                ...
                actions={(row) => (
                  <div className="flex items-center justify-end gap-2 pr-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-indigo-50 text-indigo-600 text-xs font-bold hover:bg-indigo-100 border border-indigo-100 transition-all active:scale-95 duration-100"
                    >
              ...
              <DataTable
                ...
                actions={(row) => (
                  <div className="flex items-center justify-end gap-2 pr-2">
                    <button
                      onClick={() => { setSelectedApplicant(row); setIsDetailOpen(true) }}
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-slate-100 border border-slate-200 transition-all active:scale-95 duration-100"
                    >
                      <span className="material-symbols-outlined" style={{ fontSize: 14 }}>visibility</span>
                      Lihat Formulir
                    </button>
      ...
      {selectedApplicant && (
        <Modal
          ...
          subtitle="Formulir detail pengajuan rekrutmen anggota baru."
        ...
        >
          <ModalBody>
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 bg-slate-50 border border-slate-200 p-4 rounded-2xl">
                <div className="w-14 h-14 rounded-2xl bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xl shrink-0">
                ...
                <div>
                  <h3 className="font-extrabold text-[#171717] text-base leading-snug">{selectedApplicant.Mahasiswa?.Nama}</h3>
                  <p className="text-xs text-slate-400 font-semibold mt-0.5">NIM: {selectedApplicant.Mahasiswa?.NIM || '—'}</p>
                  <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-wider">{selectedApplicant.Mahasiswa?.ProgramStudi?.Nama || 'Program Studi'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-black text-slate-400 tracking-wider uppercase mb-1">IPK Resmi (SIAKAD)</p>
                  <p className="text-base font-black text-slate-800">{selectedApplicant.IPK ? selectedApplicant.IPK.toFixed(2) : (selectedApplicant.Mahasiswa?.IPK ? selectedApplicant.Mahasiswa.IPK.toFixed(2) : '—')}</p>
                </div>
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-black text-slate-400 tracking-wider uppercase mb-1">Status Pengajuan</p>
                ...
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-black text-slate-400 tracking-wider uppercase mb-1">Divisi Pilihan Utama (1)</p>
                  <Badge className="bg-indigo-50 text-indigo-700 font-black text-[9.5px] border border-indigo-100 rounded-lg px-2.5 py-0.5 uppercase tracking-wide">
                    {selectedApplicant.Divisi || 'Umum'}
                  </Badge>
                </div>
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <p className="text-[10px] font-black text-slate-400 tracking-wider uppercase mb-1">Divisi Pilihan Alternatif (2)</p>
                  {selectedApplicant.divisi_pilihan_dua || selectedApplicant.DivisiPilihanDua ? (
                    <Badge className="bg-purple-50 text-purple-700 font-black text-[9.5px] border border-purple-100 rounded-lg px-2.5 py-0.5 uppercase tracking-wide">
                      {selectedApplicant.divisi_pilihan_dua || selectedApplicant.DivisiPilihanDua}
                    </Badge>
                  ) : (
                    <span className="text-slate-400 text-xs font-semibold italic">Tidak ada pilihan kedua</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1 ml-1">
                <p className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase font-headline">Motivasi &amp; Alasan Bergabung</p>
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold text-slate-700 leading-relaxed whitespace-pre-wrap font-body max-h-36 overflow-y-auto">
              ...
              {selectedApplicant.cv_url || selectedApplicant.CVURL ? (
                <div className="space-y-2 ml-1">
                  <p className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase font-headline">Tautan Berkas Pendukung (CV / Portfolio)</p>
                  <a
                    href={selectedApplicant.cv_url || selectedApplicant.CVURL}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-between p-3.5 bg-indigo-50/50 border border-indigo-150 rounded-2xl hover:bg-indigo-100/50 transition-all group"
                  >
                    ...
                    <span className="material-symbols-outlined text-slate-400 group-hover:text-indigo-600 transition-colors">chevron_right</span>
                  </a>
                </div>
              ) : (
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-400 italic text-center font-semibold">
          ...
          <ModalFooter>
            <ModalBtn variant="ghost" type="button" onClick={() => { setIsDetailOpen(false); setSelectedApplicant(null); }}>
              Tutup
            </ModalBtn>
            {selectedApplicant.Status?.toLowerCase() === 'pending' && (
              <div className="flex gap-2">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold border border-rose-100 transition-all active:scale-95 duration-100"
                >
                  Tolak
                </button>
                <button
                  ...
                  disabled={actionLoading}
                  className="px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 transition-all active:scale-95 duration-100 shadow-md shadow-indigo-600/10"
                >
                  Terima Pendaftaran
                </button>
              </div>
