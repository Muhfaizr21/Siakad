...
const STATUS_CONFIG = {
 diajukan: { label: 'Diajukan', cls: 'bg-blue-50 text-blue-700 ring-1 ring-blue-500/20', icon: 'schedule' },
 disetujui_dosen: { label: 'ACC Dosen', cls: 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-500/20', icon: 'check_circle' },
 disetujui_fakultas: { label: 'ACC Fakultas', cls: 'bg-violet-50 text-violet-700 ring-1 ring-violet-500/20', icon: 'check_circle' },
 disetujui_univ: { label: 'Disetujui Univ', cls: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-500/20', icon: 'check_circle' },
 revisi: { label: 'Butuh Revisi', cls: 'bg-amber-50 text-amber-700 ring-1 ring-amber-500/20', icon: 'error' },
 ditolak: { label: 'Ditolak', cls: 'bg-rose-50 text-rose-700 ring-1 ring-rose-500/20', icon: 'cancel' },
 selesai: { label: 'Selesai', cls: 'bg-slate-100 text-slate-600 ring-1 ring-slate-400/20', icon: 'check_circle' },
}

const StatusBadge = ({ status }) => {
 const cfg = STATUS_CONFIG[status] || { label: status, cls: 'bg-slate-100 text-slate-600', icon: 'description' }
 return (
...
export default function ProposalManagement() {
 ...
 label: 'Kode Ref',
 className: 'w-[120px]',
 render: (val) => <span className="font-bold text-slate-400 font-headline text-[10px] tracking-widest">PROP-{val}</span>
 },
 {
 key: 'Judul',
 label: 'Kegiatan',
 ...
 <span className="material-symbols-outlined size-4 stroke-[3px]" >add</span> Buat Proposal
 </Button>
 </div>
 </section>
 ...
 <span className="material-symbols-outlined size-32 rotate-12" >description</span>
 </div>
 <div className="relative z-10 space-y-3">
 <div className="flex items-start justify-between gap-4">
 <div>
 <p className="text-[9px] font-black text-slate-400 tracking-[0.2em] font-headline mb-1">PROP-{selected.ID}</p>
 <h2 className="text-lg font-black text-white font-headline tracking-tighter leading-tight line-clamp-2 ">{selected.Judul}</h2>
 </div>
 <StatusBadge status={selected.Status} />
 </div>
 <div className="mt-2 flex items-center gap-4">
 <div>
 <p className="text-[8px] font-black text-slate-500 tracking-widest leading-none mb-1">Anggaran</p>
 <p className="text-xs font-black text-emerald-400 font-headline">{formatRupiah(selected.Anggaran)}</p>
 </div>
 <div className="h-6 w-px bg-slate-700" />
 <div>
 <p className="text-[8px] font-black text-slate-500 tracking-widest leading-none mb-1">Tanggal</p>
 ...
 {selected.Catatan && (
 <div>
 <p className="text-[9px] font-black text-slate-400 tracking-widest mb-2 font-headline">Tujuan / Deskripsi</p>
 ...
 {/* Alur Persetujuan */}
 <div>
 <p className="text-[9px] font-black text-slate-400 tracking-widest mb-3 font-headline">Alur Persetujuan</p>
 <div className="flex items-center gap-2">
 ...
 <span className="material-symbols-outlined size-3.5" >check_circle</span>
 </div>
 <span className="text-[8px] font-black tracking-tight text-slate-500 font-headline text-center leading-none">{['Dosen', 'Fakultas', 'Univ'][i]}</span>
 </div>
 ...
 {/* Histori */}
 <div>
 <p className="text-[9px] font-black text-slate-400 tracking-widest mb-3 font-headline">Riwayat Status</p>
 ...
 <span className="material-symbols-outlined size-4 text-primary" >description</span>
 </div>
 <div className="flex-1 bg-slate-50 rounded-2xl p-3 border border-slate-100">
 <div className="flex items-center gap-2 mb-1">
 <StatusBadge status={log.Status} />
 <span className="text-[8px] text-slate-400 font-bold tracking-widest leading-none">{new Date(log.CreatedAt).toLocaleString('id-ID')}</span>
 </div>
 ...
 <h3 className="text-[10px] font-black text-slate-400 tracking-widest font-headline">Panel Kendali Status</h3>
 <Textarea
 rows={5}
 ...
 <Button disabled={isSubmitting} onClick={() => handleAction('disetujui_dosen')} className="w-full h-10 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] tracking-widest">
 {isSubmitting ? <span className="material-symbols-outlined size-3 animate-spin" >sync</span> : 'ACC TAHAP 1'}
 </Button>
 )}
 {selected.Status === 'disetujui_dosen' && (
 <Button disabled={isSubmitting} onClick={() => handleAction('disetujui_fakultas')} className="w-full h-10 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-black text-[10px] tracking-widest">
 {isSubmitting ? <span className="material-symbols-outlined size-3 animate-spin" >sync</span> : 'ACC TAHAP 2'}
 </Button>
 )}
 {selected.Status === 'disetujui_fakultas' && (
 <Button disabled={isSubmitting} onClick={() => handleAction('disetujui_univ')} className="w-full h-10 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] tracking-widest">
 {isSubmitting ? <span className="material-symbols-outlined size-3 animate-spin" >sync</span> : 'ACC FINAL (CAIR)'}
 </Button>
 )}
 <Button onClick={() => handleAction('revisi')} variant="outline" className="w-full h-10 rounded-xl border-amber-200 bg-amber-50 text-amber-800 hover:bg-amber-100 font-black text-[10px] tracking-widest">
 MINTA REVISI
 </Button>
 <Button onClick={() => handleAction('ditolak')} variant="outline" className="w-full h-10 rounded-xl border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100 font-black text-[10px] tracking-widest">
 TOLAK PERMANEN
 </Button>
 </div>
 <div className="mt-auto pt-4">
 <Button variant="ghost" onClick={() => { setIsDetailOpen(false); setKomentar('') }} className="w-full text-[10px] font-black tracking-widest text-slate-400 hover:text-slate-900 h-9 rounded-xl">
 TUTUP PANEL
 </Button>
 </div>
 </div>
 ...
 <span className="material-symbols-outlined size-24 rotate-12" >description</span>
 </div>
 ...
 <Badge variant="secondary" className="text-[9px] font-black tracking-widest px-2.5 py-0.5 bg-primary/5 text-primary border-none">
 Proposal Registry
 </Badge>
 </div>
 <DialogTitle className="text-lg md:text-2xl font-black font-headline tracking-tighter text-slate-900 ">
 ...
 <div className="space-y-2">
 <Label className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Judul / Nama Kegiatan</Label>
 <Input
 required
 ...
 <div className="space-y-2">
 <Label className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Tanggal Kegiatan</Label>
 <Input
 required
 type="date"
 ...
 <div className="space-y-2">
 <Label className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Anggaran (Rp)</Label>
 <Input
 required
 type="number"
 ...
 <div className="space-y-2">
 <Label className="text-[9px] md:text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Tujuan / Deskripsi</Label>
 <Textarea
 required
 ...
 <Button type="button" variant="ghost" onClick={() => setIsCrudOpen(false)} className="w-full md:w-auto text-[9px] md:text-[10px] font-black tracking-widest text-slate-400 hover:text-slate-900 px-8 h-10 md:h-12 rounded-xl md:rounded-2xl font-headline">
 Batalkan
 </Button>
 ...
 <span className="text-[9px] md:text-[10px] font-black tracking-[0.2em] font-headline">
