...
const OrgCard = ({ member, color = 'bg-primary/10', textColor = 'text-primary', size = 'md' }) => {
 ...
 <AvatarFallback className={cn('font-black ', color, textColor)}>
 ...
 <p className="text-[9px] font-bold text-slate-400 tracking-widest">{member.Role}</p>
 </div>
 </div>
 )
}
...
export default function StrukturOrganisasi() {
 ...
 <Button onClick={() => setIsAddDivOpen(true)} className="h-12 px-6 rounded-2xl bg-[#00236F] text-white hover:bg-[#003399] font-black text-[10px] tracking-widest shadow-xl shadow-blue-900/20 gap-2 w-full md:w-auto shrink-0">
 <span className="material-symbols-outlined size-4 stroke-[3px]" >add</span> Tambah Divisi
 </Button>
 </div>
 </section>
 ...
 {/* ── Content Area ───────────────────────────────────────────── */}
 {loading ? (
 <div className="flex items-center justify-center h-64"><div className="size-8 border-4 border-[#00236F]/30 border-t-[#00236F] rounded-full animate-spin" /></div>
 ) : (
 ...
 <p className="text-[9px] font-black text-slate-400 tracking-[0.3em] font-headline">Pengurus Inti</p>
 <div className="h-px flex-1 bg-slate-100" />
 </div>
 ...
 <div className="flex items-center gap-2">
 <div className="h-px flex-1 bg-slate-100" />
 <p className="text-[9px] font-black text-slate-400 tracking-[0.3em] font-headline">Divisi</p>
 <div className="h-px flex-1 bg-slate-100" />
 </div>
 ...
 <p className="text-[9px] font-bold text-slate-400 tracking-widest">{divMembers.length} anggota</p>
 </div>
 <Button onClick={() => setDelDiv(div)} variant="ghost" size="icon" className="h-7 w-7 hover:text-rose-600 hover:bg-rose-50 rounded-xl">
 <span className="material-symbols-outlined size-3.5" >delete</span>
 </Button>
 </div>
 <div className="flex flex-wrap gap-2">
 {divMembers.length === 0
 ? <p className="text-[9px] font-bold text-slate-300 tracking-widest">Belum ada anggota</p>
 : divMembers.map(m => (
 <div key={m.ID} className="flex items-center gap-1.5 bg-slate-50 rounded-xl px-2.5 py-1.5 border border-slate-100">
 <div className="size-5 rounded-lg bg-primary/10 text-primary flex items-center justify-center text-[8px] font-black ">
 ...
 <p className="text-[11px] font-black text-slate-400 tracking-widest text-center">Belum ada data struktur organisasi.<br />Tambahkan anggota terlebih dahulu.</p>
 </CardContent>
 </Card>
 )}
 </div>
 ...
 <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none"><span className="material-symbols-outlined size-24 rotate-12">account_tree</span></div>
 <div className="relative z-10">
 <div className="flex items-center gap-3 mb-1 md:mb-2">
 <div className="size-8 rounded-xl bg-primary/10 flex items-center justify-center text-primary"><span className="material-symbols-outlined size-4 stroke-[3px]" >add</span></div>
 <Badge className="text-[9px] font-black tracking-widest px-2.5 py-0.5 bg-primary/5 text-primary border-none">Division Registry</Badge>
 </div>
 <DialogTitle className="text-lg md:text-2xl font-black font-headline tracking-tighter text-slate-900 ">Tambah Divisi Baru</DialogTitle>
 ...
 <Button type="button" variant="ghost" onClick={() => setIsAddDivOpen(false)} className="w-full md:w-auto text-[9px] md:text-[10px] font-black tracking-widest text-slate-400 hover:text-slate-900 px-8 h-10 md:h-12 rounded-xl md:rounded-2xl flex-1">Batalkan</Button>
 ...
 <span className="text-[9px] md:text-[10px] font-black tracking-[0.2em]">Simpan Divisi</span>
 </Button>
 </DialogFooter>
 </form>
 </DialogContent>
