...
export default function JadwalKegiatan() {
 ...
 <span className="text-[10px] text-slate-400 font-bold tracking-tight mt-0.5 flex items-center gap-1">
 <span className="material-symbols-outlined size-2.5 opacity-60" >location_on</span> {row.Lokasi || 'Belum ditentukan'}
 </span>
 </div>
 )
 },
 ...
 Manajemen agenda operasional dan pemantauan jadwal kegiatan rutin organisasi.
 </p>
 </div>
 ...
 <span className="material-symbols-outlined size-4 stroke-[3px]" >add</span> Tambah Kegiatan
 </Button>
 </div>
 </section>
 ...
 <div className="flex items-center gap-2"><span className="material-symbols-outlined size-3 text-slate-400" >schedule</span><span className="text-[10px] font-bold text-slate-300">{selected.TanggalMulai ? new Date(selected.TanggalMulai).toLocaleDateString('id-ID') : '—'}</span></div>
 <div className="flex items-center gap-2"><span className="material-symbols-outlined size-3 text-slate-400" >location_on</span><span className="text-[10px] font-bold text-slate-300">{selected.Lokasi || 'Belum ditentukan'}</span></div>
 </div>
 </div>
 </div>
 ...
 {selected.Deskripsi && (
 <div>
 <p className="text-[9px] font-black text-slate-400 tracking-widest mb-2 font-headline">Deskripsi</p>
 ...
 <Button variant="ghost" onClick={() => setIsDetailOpen(false)} className="text-[10px] font-black text-slate-400 px-8 h-10 rounded-2xl">Tutup</Button>
 <Button onClick={() => { setIsDetailOpen(false); handleOpenEdit(selected) }} className="text-[10px] font-black h-10 px-8 rounded-2xl bg-primary text-white">Edit</Button>
 </div>
 </div>
 </div>
 )}
 ...
 <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none"><span className="material-symbols-outlined size-24 rotate-12" >calendar_month</span></div>
 <div className="relative z-10">
 ...
 <Badge className="text-[9px] font-black tracking-widest px-2.5 py-0.5 bg-primary/5 text-primary border-none">Event Registry</Badge>
 </div>
 <DialogTitle className="text-lg md:text-2xl font-black font-headline tracking-tighter text-slate-900 ">{isEditMode ? 'Edit Kegiatan' : 'Jadwalkan Kegiatan'}</DialogTitle>
 ...
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Nama Kegiatan</Label>
 ...
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Tanggal Mulai</Label>
 <Input required type="date" value={form.TanggalMulai} onChange={e => setForm({ ...form, TanggalMulai: e.target.value })}
 className="h-12 rounded-2xl border-slate-200 bg-slate-50/50 focus:bg-white transition-all font-bold text-sm font-headline" />
 </div>
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Tanggal Selesai</Label>
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Lokasi</Label>
 <Input value={form.Lokasi} onChange={e => setForm({ ...form, Lokasi: e.target.value })} placeholder="Gedung / Ruang..."
 className="h-12 rounded-2xl border-slate-200 bg-slate-50/50 focus:bg-white transition-all font-bold text-sm font-headline" />
 </div>
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Status</Label>
 ...
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Deskripsi</Label>
 ...
 <span className="text-[9px] md:text-[10px] font-black tracking-[0.2em]">{isSubmitting ? 'Simpan Data...' : (isEditMode ? 'Perbarui Jadwal' : 'Simpan Jadwal')}</span>
 </Button>
 </DialogFooter>
 </form>
 </DialogContent>
