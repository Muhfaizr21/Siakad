...
export default function AbsensiKegiatan() {
 ...
 <span className="text-[10px] text-slate-400 font-bold tracking-tight mt-0.5">{row.TanggalMulai ? new Date(row.TanggalMulai).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}</span>
 </div>
 )
 },
 {
 ...
 Manajemen kehadiran anggota per kegiatan dengan pemindaian kode QR instan.
 </p>
 </div>
 </section>
 ...
 actions={(row) => (
 <Button onClick={() => handleSelectEvent(row)} size="sm" className={cn('h-8 px-4 rounded-xl text-[10px] font-black ', selectedEvent?.ID === row.ID ? 'bg-primary text-white' : 'bg-primary/10 text-primary hover:bg-primary/20')}>
 ...
 <span className="material-symbols-outlined size-12 stroke-[1px]" Check>calendar_month</span>
 <p className="text-[11px] font-black tracking-widest text-center text-slate-400">Pilih kegiatan untuk<br />melihat & mencatat absensi</p>
 </div>
 ) : (
 ...
 <span className="text-[10px] font-black text-emerald-600 tracking-widest">Hadir: {attendedCount}</span>
 <span className="text-[10px] font-black text-rose-500 tracking-widest">Tidak Hadir: {absentCount}</span>
 </div>
 </div>
 ...
 <span className="material-symbols-outlined size-4" >document_scanner</span> <span className="text-[10px] font-black ">Buka QR Code</span>
 </Button>
 </div>
 {loadingAtt ? (
 <div className="flex justify-center py-8"><span className="material-symbols-outlined size-6 animate-spin text-primary" >sync</span></div>
 ) : attendance.length === 0 ? (
 <div className="flex flex-col items-center py-8 gap-2">
 <span className="material-symbols-outlined size-8 text-slate-300" >group</span>
 <p className="text-[10px] font-black text-slate-400 tracking-widest">Belum ada data absensi</p>
 </div>
 ) : (
 ...
 <AvatarFallback className="bg-slate-100 text-slate-700 text-[10px] font-black ">
 ...
 <p className="text-[9px] text-slate-400 font-bold ">{att.Mahasiswa?.NIM}</p>
 </div>
 ...
 <span className="material-symbols-outlined size-4" >check_circle</span>
 </button>
 ...
 <span className="material-symbols-outlined size-4" Circle>close</span>
 </button>
 </div>
 </div>
 ))}
 ...
 <h3 className="text-xl font-black text-slate-900 font-headline tracking-tighter ">{selectedEvent?.Judul}</h3>
 <p className="text-[10px] font-bold text-slate-400 tracking-widest">Scan QR ini untuk melakukan presensi</p>
 </div>
 ...
 <div className="absolute inset-0 bg-primary/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-[1.8rem]">
 <span className="material-symbols-outlined size-12 text-primary animate-pulse" >document_scanner</span>
 </div>
 </div>

 <div className="w-full space-y-3">
 <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10 text-center">
 <p className="text-[9px] font-black text-primary tracking-[0.2em]">Kode Keamanan Aktif</p>
 <p className="text-[11px] font-bold text-slate-600 mt-1 tracking-tighter">Berlaku untuk seluruh anggota terdaftar</p>
 </div>
 <Button onClick={() => setIsQrOpen(false)} className="w-full h-12 rounded-2xl bg-slate-900 text-white font-black text-[10px] tracking-widest">Tutup QR</Button>
 </div>
 </div>
 </DialogContent>
 </Dialog>
