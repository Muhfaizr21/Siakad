...
export default function RoleBasedAccess() {
 ...
 render: v => <span className="font-black text-slate-900 font-headline text-[13px] tracking-tighter ">{v || '—'}</span>
 },
 {
 ...
 {(v || []).slice(0, 3).map(h => (
 <Badge key={h} className="bg-primary/5 text-primary font-black text-[7px] border-none tracking-widest">
 ...
 Manajemen Role dan konfigurasi otorisasi akses modul bagi anggota Ormawa.
 </p>
 </div>
 ...
 <span className="material-symbols-outlined size-4 stroke-[3px]" >add</span> Buat Role Baru
 </Button>
 </div>
 </section>
 ...
 <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none"><span className="material-symbols-outlined size-24 rotate-12" >security</span></div>
 ...
 <Badge className="text-[9px] font-black tracking-widest px-2.5 py-0.5 bg-primary/5 text-primary border-none">RBAC Registry</Badge>
 </div>
 <DialogTitle className="text-lg md:text-2xl font-black font-headline tracking-tighter text-slate-900 ">{isEditMode ? 'Edit Role' : 'Buat Role Baru'}</DialogTitle>
 <DialogDescription className="text-[10px] md:text-xs font-medium text-slate-400 mt-1">Definisikan role dan hak akses sistem untuk anggota ormawa.</DialogDescription>
 </div>
 </DialogHeader>
 <form onSubmit={handleSave} className="p-4 md:p-8 pt-3 md:pt-6 space-y-3 md:space-y-4">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="space-y-2">
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Nama Role</Label>
 ...
 <Label className="text-[10px] font-black text-slate-400 tracking-[0.2em] ml-1 font-headline">Pilih Hak Akses (Centang Semua yang Berlaku)</Label>
 ...
 <span className={cn('text-[9px] font-black tracking-tight leading-tight', form.Hak.includes(p) ? 'text-primary' : 'text-slate-500')}>{PERM_LABELS[p]}</span>
 </label>
 ))}
 </div>
 </div>
 ...
 <span className="text-[9px] md:text-[10px] font-black tracking-[0.2em]">{isEditMode ? 'Update Role' : 'Buat Role'}</span>
 </Button>
 </DialogFooter>
 </form>
 </DialogContent>
