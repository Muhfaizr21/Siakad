    ...
    <div className="bg-white p-8 rounded-3xl border border-[#e5e5e5] shadow-sm mb-12">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-xl font-extrabold font-headline flex items-center gap-3">
          <Bell size={24} className="text-[#f97316]" />
          Pengumuman Terbaru
        </h2>
        <NavLink to="/student/dashboard" className="text-sm font-bold text-[#f97316] hover:underline flex items-center gap-1 group">
          Lihat Semua
          ...
          {announcements.map((item, idx) => (
            <div key={idx} className="flex flex-col gap-4 group/item">
              <div className="flex items-center justify-between">
                <span className="px-2.5 py-1 bg-[#fff7ed] text-[#f97316] rounded-lg text-[10px] font-black uppercase tracking-widest border border-orange-100">
                  {item.kategori}
                </span>
                <span className="text-[10px] font-bold text-[#a3a3a3] uppercase">{item.tanggal}</span>
              </div>
              <div>
                <h4 className="font-bold text-lg mb-2 group-hover/item:text-[#f97316] transition-colors line-clamp-1">{item.judul}</h4>
                ...
                <NavLink 
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-black text-[#171717] hover:text-[#f97316] uppercase tracking-widest transition-colors decoration-2 underline-offset-4 hover:underline"
                >
