...
func getEventsForMonth(student *models.Mahasiswa, bulan int, tahun int) []Event {
	...
	for _, k := range pkkmbKeg {
		events = append(events, Event{Tanggal: k.Tanggal, Judul: "PKKMB: " + k.Judul, Kategori: "pkkmb"})
	}
	...
	for _, e := range ormawaEvents {
		events = append(events, Event{Tanggal: e.TanggalMulai, Judul: "Event: " + e.Judul, Kategori: "organisasi"})
	}
