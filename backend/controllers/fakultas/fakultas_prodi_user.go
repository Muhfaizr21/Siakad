...
func CreateProdiAdmin(c *fiber.Ctx) error {
	...
	// Verify the program studi belongs to this faculty
	var prodi models.ProgramStudi
	if err := config.DB.Where("id = ? AND fakultas_id = ?", req.ProgramStudiID, fid).First(&prodi).Error; err != nil {
...
func UpdateProdiAdmin(c *fiber.Ctx) error {
	...
	if err := config.DB.Where("id = ? AND fakultas_id = ? AND role = ?", id, fid, "prodi_admin").First(&user).Error; err != nil {
	...
	// Update program studi (verify it belongs to this faculty)
	if req.ProgramStudiID != 0 {
		var prodi models.ProgramStudi
		if err := config.DB.Where("id = ? AND fakultas_id = ?", req.ProgramStudiID, fid).First(&prodi).Error; err != nil {
			return c.Status(400).JSON(fiber.Map{"status": "error", "message": "Program Studi tidak valid atau bukan milik fakultas ini"})
