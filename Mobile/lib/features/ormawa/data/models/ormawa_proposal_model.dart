...
class OrmawaProposalModel extends OrmawaProposal {
  ...
  factory OrmawaProposalModel.fromJson(Map<String, dynamic> json) {
    return OrmawaProposalModel(
      ...
      mahasiswaId: json['MahasiswaID']?.toString() ?? json['mahasiswa_id']?.toString(),
      fakultasId: json['FakultasID']?.toString() ?? json['fakultas_id']?.toString(),
      title: json['Judul'] ?? json['title'] ?? '',
      code: 'PROP-${json['ID']}',
      status: (json['Status'] ?? json['status'] ?? 'diajukan').toString().toLowerCase(),
      date: json['TanggalKegiatan'] != null 
          ? DateTime.parse(json['TanggalKegiatan']) 
          : (json['date'] != null ? DateTime.parse(json['date']) : DateTime.now()),
      budget: (json['Anggaran'] as num?)?.toDouble() ?? (json['budget'] as num?)?.toDouble() ?? 0,
  ...
  Map<String, dynamic> toJson() {
    ...
    if (ormawaId != null && ormawaId!.isNotEmpty) data['OrmawaID'] = int.tryParse(ormawaId!);
    if (mahasiswaId != null && mahasiswaId!.isNotEmpty) data['MahasiswaID'] = int.tryParse(mahasiswaId!);
    if (fakultasId != null && fakultasId!.isNotEmpty) data['FakultasID'] = int.tryParse(fakultasId!);
