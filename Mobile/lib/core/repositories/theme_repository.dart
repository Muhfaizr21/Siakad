...
class ThemeRepository {
  ...
  ThemeRepository({ApiClient? apiClient}) : _apiClient = apiClient ?? ApiClient();

  /// Get theme from API with local cache fallback
  /// Returns the theme version along with colors
  Future<({MobileThemeColors colors, String version})> getThemeWithVersion() async {
    try {
  ...
  Future<({MobileThemeColors colors, String version})?> _getCachedThemeWithVersion() async {
    try {
      final prefs = await SharedPreferences.getInstance();
