...
class ProposalPdfService {
  static Future<void> generateAndPrintPdf(OrmawaProposal proposal, {String ormawaName = 'Organisasi Kemahasiswaan'}) async {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            _buildInfoRow('Landasan Kegiatan', proposal.landasanKegiatan ?? '-'),
            ...
            _buildInfoRow('Jadwal Pelaksanaan', proposal.jadwalPelaksanaan ?? '-'),
            _buildInfoRow('Total Anggaran', 'Rp ${NumberFormat('#,###', 'id_ID').format(proposal.budget)}'),
            _buildInfoRow('Sumber Dana', proposal.sumberDana ?? '-'),
            pw.SizedBox(height: 16),
            _buildSectionTitle('Deskripsi Kegiatan'),
            pw.Text(proposal.description ?? '-', style: const pw.TextStyle(fontSize: 11)),
            pw.SizedBox(height: 16),
            _buildSectionTitle('Latar Belakang'),
            pw.Text(proposal.latarBelakang ?? '-', style: const pw.TextStyle(fontSize: 11)),
            pw.SizedBox(height: 16),
            _buildSectionTitle('Tujuan'),
            pw.Text(proposal.tujuanKegiatan ?? '-', style: const pw.TextStyle(fontSize: 11)),
            pw.SizedBox(height: 16),
            _buildSectionTitle('Indikator Keberhasilan'),
            pw.Text(proposal.indikatorKeberhasilan ?? '-', style: const pw.TextStyle(fontSize: 11)),
    ...
    await Printing.sharePdf(bytes: bytes, filename: 'Proposal_${proposal.code}.pdf');
  }
  ...
  static pw.Widget _buildHeader() {
    return pw.Center(
      child: pw.Column(
        children: [
          pw.Text('FORM PENGAJUAN PROPOSAL KEGIATAN', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 4),
          pw.Text('KEMAHASISWAAN UNIVERSITAS BINA KARYA', style: const pw.TextStyle(fontSize: 12)),
    ...
    return pw.Column(
      children: [
        ...
        pw.Row(
          ...
          children: [
            _buildSigBox('Menyetujui:\nPembina Organisasi', '____________________'),
            _buildSigBox('Menyetujui:\nKepala Divisi Kemahasiswaan', '____________________'),
          ],
        ),
      ],
    );
  ...
  static pw.Widget _buildSigBox(String role, String name) {
    return pw.Column(
      ...
      children: [
        pw.Text(role, textAlign: pw.TextAlign.center, style: const pw.TextStyle(fontSize: 10)),
        pw.SizedBox(height: 40),
